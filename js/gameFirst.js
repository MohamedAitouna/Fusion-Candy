var game = new Phaser.Game(600, 480, Phaser.CANVAS,null, { preload: preload, create: create });
...
var scorrre=-25;
function preload() {
	game.load.image('backdrop', 'assets/img/digiBackground.jpg');
	game.load.image('backobj', 'assets/img/backObje.png');
	game.load.image('restart', 'assets/img/res.png');
	game.load.image('sccore', 'assets/img/s.png');
    game.load.spritesheet("GEMS", "assets/img/all.png", FRAME_SIZE, FRAME_SIZE);
...
function create() {
	...
	back=game.add.sprite(0, 0, 'backdrop');
	back.width=600;
	back.height=480;
	back.alpha=0.2;
	
	
	res=game.add.sprite(500, 40, 'restart');
	res.width=40;
	res.height=40;
...
function tweenGemPos(gem, newPosX, newPosY, durationMultiplier) {
   ...
   // console.log('Tween ',gem.name,' from ',gem.posX, ',', gem.posY, ' to ', newPosX, ',', newPosY);
    ...
    return game.add.tween(gem).to({x: (newPosX  * FRAME_SIZE_SPACED)+FRAME_SIZE_SPACED, y: (newPosY * FRAME_SIZE_SPACED)+FRAME_SIZE_SPACED}, speed * durationMultiplier, Phaser.Easing.Linear.None, true);
...
function spawnBoard() {
    ...
    for (var i = 0; i < BOARD_COLS; i++)
    {
        for (var j = 0; j < BOARD_ROWS; j++)
        {
			...
			game.add.sprite((i*FRAME_SIZE_SPACED)+FRAME_SIZE_SPACED,(j*FRAME_SIZE_SPACED)+FRAME_SIZE_SPACED, 'backobj');
            ...
            gem.events.onInputDown.add(onObjectInputDown, this);
            gem.events.onInputUp.add(onObjectInputUp, this);
    ...
    game.time.events.add(dropGemDuration * speed, refillBoard);
	//game.time.events.add(speed, function(){console.log("yes yes yes")});
...
function refillBoard() {
    ...
    game.time.events.add(maxGemsMissingFromCol * 2 * speed, boardRefilled);
...
function boardRefilled() {
	//game.time.events.add(1000,deadpool);
	deadpool();
	removeKilledGems();
	var dropGemDuration = dropGems();
...
function moveObject(coffX,coffY){
	...
	if(ourObject.y==objectY && ourObject.x==objectX ){
		...
		tw1=game.add.tween(ourObject).to({x:ourObject.x+(coffX*FRAME_SIZE_SPACED)-(1*coffX),y: ourObject.y+(coffY*FRAME_SIZE_SPACED)-(1*coffY)}, speed, Phaser.Easing.Linear.None, true);
		tw2=game.add.tween(newObject).to({x:ourObject.x-(1*coffX),y: ourObject.y-(1*coffY)}, speed, Phaser.Easing.Linear.None, true);
...
function mouseMove(pointer, x, y){
	if(onObject && ourObject.x%FRAME_SIZE_SPACED==0 && ourObject.y%FRAME_SIZE_SPACED==0){
		...
		if(game.input.mousePointer.x<ourObject.x && ourObject.y==objectY){
			if(ourObject.x>=objectX && ourObject.posX>0){
				moveObject(-1,0);
			}
		}
		else if(game.input.mousePointer.x>ourObject.x+FRAME_SIZE_SPACED && ourObject.y==objectY){
			if(ourObject.x<=objectX && ourObject.posX<BOARD_COLS-1){
				moveObject(1,0);
			}
		}
		else if(game.input.mousePointer.y<ourObject.y && ourObject.posY>0 ){
			if(ourObject.y>=objectY && ourObject.x==objectX){
				moveObject(0,-1);
			}
		}
		else if (game.input.mousePointer.y>ourObject.y+FRAME_SIZE_SPACED && ourObject.x==objectX){
...
function onObjectInputUp(){
	...
	if(ourObject.x!==objectX || ourObject.y!==objectY ){
		...
		if( o1 || o2 ){
		 ...
		 game.tweens.remove(tw1);
		 game.tweens.remove(tw2); 
	     game.add.tween(newObject).to({x:newObjectX,y:newObjectY}, speed, Phaser.Easing.Linear.None, true);
	     game.add.tween(ourObject).to({x: objectX,y:objectY}, speed, Phaser.Easing.Linear.None, true);
...
groupeObject.removeAll();
 spawnBoard();
		...
		/*console.log("left");
				var px=ourObject.posX;
				var py=ourObject.posY;
				newObject=getGem(ourObject.posX-1,ourObject.posY);
				if(ourObject.x==objectX){
				newObjectX=ourObject.x-FRAME_SIZE_SPACED;
				newObjectY=ourObject.y;
				idX=ourObject.posX-1;
				idY=ourObject.posY;
				}else{
				newObjectX=ourObject.x;
				newObjectY=ourObject.y;
				...
				ourObject.x-=1;
			    tw1=game.add.tween(ourObject).to({x: ourObject.x-FRAME_SIZE_SPACED+1}, speed, Phaser.Easing.Linear.None, true);
			    tw2=game.add.tween(newObject).to({x: ourObject.x+1}, speed, Phaser.Easing.Linear.None, true);
				setObjectPos(newObject, px, py);
				setObjectPos(ourObject, px-1, py);*/
				...
				var px=ourObject.posX;
				var py=ourObject.posY;
				newObject=getGem(ourObject.posX,ourObject.posY-1);
				if(ourObject.y==objectY){
				newObjectX=ourObject.x;
				newObjectY=ourObject.y-FRAME_SIZE_SPACED;
				idX=ourObject.posX;
				idY=ourObject.posY-1;
				}else{
				newObjectX=ourObject.x;
				newObjectY=ourObject.y;
				idX=ourObject.posX;
			...
			ourObject.y-=1;
			tw1=game.add.tween(ourObject).to({y: ourObject.y-FRAME_SIZE_SPACED+1}, speed, Phaser.Easing.Linear.None, true);
			tw2=game.add.tween(newObject).to({y: ourObject.y+1}, speed, Phaser.Easing.Linear.None, true);
			setObjectPos(newObject,px, py);
			setObjectPos(ourObject, px, py-1);*/
