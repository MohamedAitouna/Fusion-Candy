...
GameOne.Levels.prototype = {
	...
	create: function () {
		...
		for (let i = 0; i < 10; i++) {
			...
			this.add.tween(key.scale).to({
				x: 0.7,
				y: 0.7
			}, 3000, Phaser.Easing.Quartic.InOut, true, 0, 10, true);
		}
		...
		this.level1E = this.add.sprite(this.game.width * ratioW[GameOne.mainObjects.lvl], this.game.height * ratioH[GameOne.mainObjects.lvl], 'effects');
		this.level1E.anchor.setTo(0.5);
		this.level1E.frame = 0;
		this.add.tween(this.level1E.scale).to({
			x: 0.7,
			y: 0.7
		}, 1500, Phaser.Easing.Quartic.InOut, true, 0, 10, true);
		for (let i = GameOne.mainObjects.lvl+1; i < 6; i++) {
			putLevelsEffect(i)
		}
...
function onLevel1InputUp(item) {
	numbOfFram=item.valI+3;
	if (numbOfFram < 9 && GameOne.mainObjects.lvl>=item.valI) {
		  GameOne.mainObjects.currentLvl=item.valI;
...
function resurrect() {
	...
	if (numStar < 20) {
		...
		key.scale.setTo(0);
		key.anchor.setTo(0.5);
		__thisLevels.add.tween(key.scale).to({
			x: 0.7,
			y: 0.7
		}, 3000, Phaser.Easing.Quartic.InOut, true, 0, 10, true);
	}
