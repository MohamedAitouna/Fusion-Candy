...
function tweenGemPos(gem, newPosX, newPosY, durationMultiplier) {
   ...
   // //console.log('Tween ',gem.name,' from ',gem.posX, ',', gem.posY, ' to ', newPosX, ',', newPosY);
    ...
    return __this.add.tween(gem).to({x: (newPosX  * FRAME_SIZE_SPACED)+FRAME_SIZE_SPACED*space_grid_x, y: (newPosY * FRAME_SIZE_SPACED)+FRAME_SIZE_SPACED*space_grid_y}, speed * durationMultiplier, Phaser.Easing.Linear.None, true);
...
function moveObject(coffX,coffY){
	...
	if(ourObject.y==objectY2 && ourObject.x==objectX2 ){
		...
		tw1=__this.add.tween(ourObject).to({x:ourObject.x+(coffX*FRAME_SIZE_SPACED)-(1*coffX),y: ourObject.y+(coffY*FRAME_SIZE_SPACED)-(1*coffY)}, speed, Phaser.Easing.Linear.None, true);
		tw2=__this.add.tween(newObject).to({x:ourObject.x-(1*coffX),y: ourObject.y-(1*coffY)}, speed, Phaser.Easing.Linear.None, true);
		...
		if( o1 || o2 ){
			...
			if(lenX>3){
			  for(let i=0;i<lenX;i++){
				  ...
				  anim.play();
				  __this.time.events.add(frameSpeedMSeconde, function(){objectToDeathX[i].kill();});
			      //
			  }
			}
			...
			//console.log("why x2");
		      var anim;
				  ...
				  anim.play();	  
				__this.time.events.add(frameSpeedMSeconde, function(){objectToDeathX2[i].kill();});
			}
			}
			...
			if(lenY>3){
			  for(let i=0;i<lenY;i++){
				  ...
				  anim.play();
			      __this.time.events.add(frameSpeedMSeconde, function(){objectToDeathY[i].kill();});
			  }
			}
			...
			if(lenY2>3){
			  for(let i=0;i<lenY2;i++){
				  ...
				  anim.play();
			      __this.time.events.add(frameSpeedMSeconde, function(){objectToDeathY2[i].kill();});
					  }
			   }
	     ...
	     __this.add.tween(newObject).to({x:newObject2X,y:newObject2Y}, speed, Phaser.Easing.Linear.None, true);
	     __this.add.tween(ourObject).to({x: objectX2,y:objectY2}, speed, Phaser.Easing.Linear.None, true);
